    ...
    public Question createQuestion(QuestionRequest request, User currentUser) {
        ...
        if (aiEnabled) {
            try {
                ...
                logger.debug("AI mode enabled - Generating embedding for combined text ({} chars)", textToEmbed.length());
            ...
            logger.debug("AI mode disabled - Skipping embedding generation (using H2 demo mode)");
        }
        ...
        logger.debug("Broadcasting WebSocket notification for question: {}", savedQuestion.getTitle());
